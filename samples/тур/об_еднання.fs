...
module РозділяючіОб'еднання =
    ...
    let rec існує елемент бпд =
        ...
        | Вузел (значення, ліве, праве) ->
            if елемент = значення then true
            elif елемент < значення then (існує елемент ліве) // Перевірити ліве піддерево.
    ...
    let rec вставити елемент бдп =
        ...
        | Вузел(значення, ліве, праве) as вузел ->
            if елемент = значення then вузел // Не треба вставляти, він вже існує; повернути вузел.
            elif елемент < значення then Вузел(значення, вставити елемент ліве, праве) // Викликати у ліве піддерево.
            else Вузел(значення, ліве, вставити елемент праве) // Викликати у праве піддерево.
